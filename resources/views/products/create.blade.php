...
<div class="container mt-4">
    ...
    <form action="{{ route('products.store') }}" method="POST">
        ...
        <div class="mb-3">
            ...
            <input type="number" name="seuil_minimum" id="seuil_minimum" class="form-control" value="{{ old('seuil_minimum') }}" required>
            @error('seuil_minimum')
                <div class="text-danger">{{ $message }}</div>
            @enderror
        </div>
        <div class="mb-3">
            ...
            <select name="category_id" id="category_id" class="form-control" required>
                ...
                @foreach($categories as $category)
                    <option value="{{ $category->id }}" @if(old('category_id') == $category->id) selected @endif>{{ $category->nom }}</option>
                @endforeach
            </select>
            @error('category_id')
                <div class="text-danger">{{ $message }}</div>
